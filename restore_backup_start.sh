...
backup() {
  echo "$(date): Starting backup of /app/static/ to $RCLONE_REMOTE:/$BACKUP_PATH ..."
  echo "rclone copy /app/static/ $RCLONE_REMOTE:/$BACKUP_PATH"
  rclone -vv copy /app/static/ "$RCLONE_REMOTE:/$BACKUP_PATH"
  echo "$(date): Backup of /app/static/ to $RCLONE_REMOTE:/$BACKUP_PATH completed."
}
...
restore_backup() {
  ...
  if rclone -vv lsf "$RCLONE_REMOTE:$BACKUP_PATH" >/dev/null 2>&1; then
    ...
    rclone -vv copy "$RCLONE_REMOTE:$BACKUP_PATH" /app/static/
...
enable_auto_backup_if_needed() {
  ...
  if [ -z "$BACKUP_CRON" ]; then
    echo "Error: No cron expression provided for scheduling. Set BACKUP_CRON and try again."
    return 1
  fi

  BACKUP_CRON="${BACKUP_CRON:-0 5 *}"
  ...
  if [ -n "$BACKUP_HOOK" ]; then
    ...
    echo "Scheduling backups with cronlite expression: $BACKUP_CRON"
    schedule_backups &
  fi
}
